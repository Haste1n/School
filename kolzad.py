...
for i in range(0, len(wejscien)):
    if wejscien[i][0] == "Z":
        if (int(wejscien[i][1]) - 1, int(wejscien[i][2]) - 1) in wonsz:
            print(int(plansza[int(wejscien[i][1]) - 1][int(wejscien[i][2]) - 1]))
        else:
            print(-1)
    else:
        if wejscien[i][0] == "G":
        ...
        if not plansza[wonsz[-1][0]][wonsz[-1][1]]:
            for l in range(1, len(wonsz)):
                plansza[wonsz[len(wonsz) - l][0]][wonsz[len(wonsz) - l][1]] = plansza[wonsz[len(wonsz) - l-1][0]][wonsz[len(wonsz) - l-1][1]]
                plansza[wonsz[len(wonsz) - l - 1][0]][wonsz[len(wonsz) - l - 1][1]] = None
            wonsz.pop(0)